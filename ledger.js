(() => {
  ...
  function normalizeData(raw) {
    ...
    const rawManualRates = raw.settings?.manualRates && typeof raw.settings.manualRates === "object" ? raw.settings.manualRates : {};
    const manualRates = Object.fromEntries(Object.entries(rawManualRates)
      .map(([code, rate]) => [String(code).toUpperCase(), Number(rate)])
      .filter(([code, rate]) => code !== baseCurrency && CURRENCY_BY_CODE.has(code) && Number.isFinite(rate) && rate > 0 && rate < 1000000));
    const bills = (Array.isArray(raw.bills) ? raw.bills : []).flatMap((bill) => {
      ...
      return [{
        ...
        originalAmountCents,
        baseAmountCents: Number.isSafeInteger(baseAmountCents) && baseAmountCents > 0 ? baseAmountCents : null,
        currency,
        category,
    ...
    return {
      version: STORAGE_VERSION,
      settings: { baseCurrency, commonCurrencies, lastCurrency, manualRates },
      travelers,
      bills,
  ...
  let exchange = { loading: false, rates: {}, fetchedAt: null, error: "" };
  function foreignCurrencies(){return [...new Set(ledgerData.bills.map(b=>b.currency).filter(c=>c!=="CNY"))].sort();}

  function manualRate(currency){const rate=Number(ledgerData.settings?.manualRates?.[currency]);return Number.isFinite(rate)&&rate>0?rate:null;}

  function effectiveExchange(currency){if(currency==="CNY")return{rate:1,source:"base",date:""};const manual=manualRate(currency);if(manual)return{rate:manual,source:"manual",date:""};const live=exchange.rates[currency];if(Number.isFinite(live?.rate)&&live.rate>0)return{rate:live.rate,source:"live",date:live.date||""};return null;}

  function settlementReady(){return foreignCurrencies().every(c=>Boolean(effectiveExchange(c)));}

  function settlementCents(bill){if(bill.currency==="CNY")return bill.originalAmountCents;const rate=effectiveExchange(bill.currency)?.rate;if(!Number.isFinite(rate)||rate<=0)return 0;const amount=Math.round(bill.originalAmountCents*rate);if(!Number.isSafeInteger(amount))throw new Error("换算金额超出可计算范围");return amount;}

  function originalTotalsMarkup(){const totals=new Map([["CNY",0],["EUR",0]]);ledgerData.bills.forEach(b=>totals.set(b.currency,(totals.get(b.currency)||0)+b.originalAmountCents));return [...totals].map(([currency,amount])=>`<span class="ledger-original-total">${escapeHtml(currency)} ${escapeHtml(formatMoney(amount,currency))}</span>`).join("");}

  function renderManualRateEditor(currency){if(manualRateEditorCurrency!==currency)return"";const current=manualRate(currency)||exchange.rates[currency]?.rate||"";return `<form class="ledger-manual-rate-form" data-ledger-form="manual-rate" data-ledger-code="${escapeAttribute(currency)}"><label><span>1 ${escapeHtml(currency)} =</span><input class="ledger-input" name="rate" inputmode="decimal" type="number" min="0.000001" max="999999.999999" step="0.000001" value="${escapeAttribute(current)}" required><span>CNY</span></label><button class="ledger-secondary-button" type="submit">应用手动汇率</button></form>`;}

  function renderExchangePanel(){const currencies=foreignCurrencies();return `<section class="ledger-exchange-panel" aria-live="polite"><h3>原币合计</h3><div>${originalTotalsMarkup()}</div><h3>结算汇率</h3>${currencies.map(c=>{const manual=manualRate(c);const live=exchange.rates[c];const active=effectiveExchange(c);const displayRate=active?Number(active.rate).toFixed(4).replace(/0+$/,"").replace(/\.$/,""):"";const activeText=active?`<strong>1 ${escapeHtml(c)} = ${escapeHtml(displayRate)} CNY</strong><span>${active.source==="manual"?"手动汇率":"最新汇率"}</span>`:`<strong>${escapeHtml(c)} → CNY：尚未获取</strong><span>等待汇率</span>`;return `<div class="ledger-exchange-row"><div class="ledger-exchange-current">${activeText}</div>${manual&&live?`<small>最新参考：1 ${escapeHtml(c)} = ${escapeHtml(live.rate)} CNY · ${escapeHtml(live.date||"")}</small>`:live&&!manual?`<small>汇率日期：${escapeHtml(live.date||"")}</small>`:""}<div class="ledger-exchange-actions"><button type="button" class="ledger-text-button" data-ledger-action="toggle-manual-rate" data-ledger-code="${escapeAttribute(c)}">${manualRateEditorCurrency===c?"收起手动设置":"手动设置汇率"}</button>${manual?`<button type="button" class="ledger-text-button" data-ledger-action="clear-manual-rate" data-ledger-code="${escapeAttribute(c)}">恢复最新汇率</button>`:""}</div>${renderManualRateEditor(c)}</div>`;}).join("")}${exchange.fetchedAt?`<p>最新汇率调取时间：${escapeHtml(new Date(exchange.fetchedAt).toLocaleString("zh-CN"))}</p>`:""}${currencies.length?`<p class="ledger-field-help">自动汇率来自 Frankfurter / 欧洲央行日参考汇率；如使用手动汇率，最终人民币分账会立即按手动值重新计算。</p><button type="button" class="ledger-text-button" data-ledger-action="refresh-exchange" ${exchange.loading?"disabled":""}>${exchange.loading?"正在获取汇率…":"刷新最新汇率"}</button>`:"<p>全部为人民币，无需换算。</p>"}${exchange.error?`<p role="alert">${escapeHtml(exchange.error)}</p>`:""}</section>`;}
  async function fetchFrankfurterRate(currency) {
    const base = encodeURIComponent(String(currency || "").toLowerCase());
    const request = async (url, parser) => {
      const response = await fetch(url, {cache: "no-store", signal: AbortSignal.timeout(12000)});
      if (!response.ok) throw new Error(`汇率接口返回 HTTP ${response.status}`);
      const payload = await response.json();
      const row = parser(payload);
      const rate = Number(row.rate);
      const date = String(row.date || "");
      if (!Number.isFinite(rate) || rate <= 0 || !/^\d{4}-\d{2}-\d{2}$/.test(date)) throw new Error("汇率数据无效");
      return {rate, date};
    };

    try {
      return await request(
        `https://api.frankfurter.dev/v2/rate/${base}/cny`,
        payload => ({rate: payload.rate, date: payload.date})
      );
    } catch (v2Error) {
      try {
        return await request(
          `https://api.frankfurter.dev/v1/latest?base=${base.toUpperCase()}&symbols=CNY`,
          payload => ({rate: payload.rates?.CNY, date: payload.date})
        );
      } catch (v1Error) {
        const error = new Error("Frankfurter 汇率服务暂不可用");
        error.cause = {v2Error, v1Error};
        throw error;
      }
    }
  }
  ...
  async function refreshExchange() {
    ...
    exchange.loading = true; exchange.error = "";
    if (ledgerRoot) renderApp();
    try {
      const rows = await Promise.all(currencies.map(async currency => [
        currency,
        await fetchFrankfurterRate(currency)
      ]));
      exchange.rates = Object.fromEntries(rows);
      exchange.fetchedAt = new Date().toISOString();
    } catch (error) {
      console.warn("Exchange-rate refresh failed", error);
      exchange.error = "暂时无法取得最新汇率。请稍后重试；已记录的原币金额不受影响。";
    } finally {
  ...
  async function submitManualRate(form){const currency=String(form.dataset.ledgerCode||"").toUpperCase();const rate=Number(new FormData(form).get("rate"));if(!CURRENCY_BY_CODE.has(currency)||currency===ledgerData.settings.baseCurrency||!Number.isFinite(rate)||rate<=0||rate>=1000000){setNotice("请输入有效的手动汇率。");form.elements.rate?.focus();return;}exchange.error="";await mutateData(next=>{next.settings.manualRates={...(next.settings.manualRates||{}),[currency]:rate};},{reason:"manual-rate-updated",message:`已按 1 ${currency} = ${rate} CNY 重新计算分账`,afterSuccess(){manualRateEditorCurrency="";}});}

  async function clearManualRate(currency){const code=String(currency||"").toUpperCase();await mutateData(next=>{next.settings.manualRates={...(next.settings.manualRates||{})};delete next.settings.manualRates[code];},{reason:"manual-rate-cleared",message:`已恢复使用 ${code} 最新汇率`,afterSuccess(){manualRateEditorCurrency="";}});if(!exchange.rates[code])void refreshExchange();}
  ...
  function billShares(bill) {
    ...
    const share = Math.floor(settlementCents(bill) / participantIds.length);
    let remainder = settlementCents(bill) - share * participantIds.length;
  ...
  function calculateStats() {
    ...
    ledgerData.bills.forEach((bill) => {
      ...
      if (payer) {
        payer.paidCents += settlementCents(bill);
    ...
    return {
      totalCents: ledgerData.bills.reduce((sum, bill) => sum + settlementCents(bill), 0),
      members,
      transfers
    };
  }
      ...
      <section class="ledger-entry-card" aria-labelledby="ledger-bill-form-title">
        ...
        ${ledgerData.travelers.length ? `
          <form class="ledger-bill-form" data-ledger-form="bill" novalidate>
            ...
            <p class="ledger-field-help">按原币金额记账，欧元与人民币分别汇总；结算时统一换算人民币。</p>
      ...
      <article class="ledger-bill-row" data-ledger-bill-id="${escapeAttribute(bill.id)}">
        <div class="ledger-bill-main">
          ...
          <div class="ledger-bill-amount">
            ...
            <span>${escapeHtml(bill.currency)} · 原币金额</span>
          </div>
        </div>
  ...
  function renderBillList() {
    const baseCurrency = ledgerData.settings.baseCurrency;
    const totalCents = ledgerData.bills.reduce((sum, bill) => sum + settlementCents(bill), 0);
      ...
      <section class="ledger-list-section" aria-labelledby="ledger-list-title">
        <div class="ledger-section-heading ledger-list-heading">
          ...
          <div class="ledger-list-total">
            <span>总支出</span>
            <strong>${originalTotalsMarkup()}</strong>
          </div>
        </div>
        ${bills.length
  ...
  function renderRelatedBills(member){if(!member.billIds.length)return`<p class="ledger-member-empty">暂无相关账单</p>`;return member.billIds.map(billId=>{const bill=ledgerData.bills.find(entry=>entry.id===billId);if(!bill)return"";const share=billShares(bill).get(member.traveler.id)||0;const payer=travelerById(bill.payerId);const converted=settlementCents(bill);const title=bill.note||`${bill.category}账单`;const original=formatMoney(bill.originalAmountCents,bill.currency);const convertedLabel=bill.currency===ledgerData.settings.baseCurrency?"":` ≈ ${formatMoney(converted,ledgerData.settings.baseCurrency)}`;const paidByMember=bill.payerId===member.traveler.id;return `<article class="ledger-member-bill"><div class="ledger-member-bill__heading"><span><strong>${escapeHtml(title)}</strong><small>${escapeHtml(bill.category)} · ${escapeHtml(payer?.name||"")}买单</small></span>${paidByMember?"<em>我买单</em>":""}</div><div class="ledger-member-bill__amounts"><span>账单 ${escapeHtml(original)}${escapeHtml(convertedLabel)}</span>${paidByMember?`<span>实际支付 ${escapeHtml(formatMoney(converted,ledgerData.settings.baseCurrency))}</span>`:""}<strong>${share?`我的分摊 ${escapeHtml(formatMoney(share,ledgerData.settings.baseCurrency))}`:"未参与分摊"}</strong></div></article>`;}).join("");}

  function renderStatsPage() {
    if (!settlementReady()) return `<section class="ledger-tab-panel" data-ledger-panel="stats" role="tabpanel" aria-labelledby="ledger-stats-tab" ${activeTab === "stats" ? "" : "hidden"}>${renderExchangePanel()}<p class="ledger-empty-state">取得汇率后显示人民币分摊与转账金额。原币账单已保存。</p></section>`;
              ...
              ${stats.members.map((member) => `
                <details class="ledger-member-stat" open>
                  ...
                  <div class="ledger-member-stat-body">
                    ...
                    <div class="ledger-member-bills"><h4>购买 / 分摊明细</h4>${renderRelatedBills(member)}</div>
                  </div>
                </details>`).join("")}
  ...
  function backupFileName() {
    ...
    return `travel-ledger-${stamp}.json`;
  }

  function exportLedgerBackup() {
    if (!ledgerData) return;
    const payload = {
      format: "travel-ledger-backup",
      version: 1,
      tripId: ledgerTripId,
      exportedAt: new Date().toISOString(),
      data: deepClone(ledgerData)
    };
    try {
      ...
      setNotice("账本备份已导出。请妥善保存这个 JSON 文件。");
    } catch (error) {
      console.error("TravelLedger could not export backup", error);
      setNotice("导出失败，请稍后重试。");
    }
  }

  function requestLedgerImport() {
    const input = ledgerRoot?.querySelector("[data-ledger-backup-input]");
    if (!input) return;
    input.value = "";
    input.click();
  }

  async function importLedgerBackup(input) {
    ...
    try {
      ...
      console.error("TravelLedger could not parse backup", error);
      setNotice("无法读取该备份文件。请选择从本账本导出的 JSON 文件。");
      return;
    }

    const candidate = payload?.format === "travel-ledger-backup" ? payload.data : payload;
    if (!candidate
      || typeof candidate !== "object"
      || !candidate.settings
      || !Array.isArray(candidate.travelers)
      || !Array.isArray(candidate.bills)) {
      setNotice("备份文件格式不正确，未导入任何数据。");
      return;
    }

    const normalized = normalizeData(candidate);
    const summary = `将导入 ${normalized.travelers.length} 位成员、${normalized.bills.length} 笔账单，并覆盖当前浏览器中的账本。继续吗？`;
    ...
    try {
      ...
      billDraft = null;
      openDialogName = null;
      notice = `导入完成：${ledgerData.travelers.length} 位成员、${ledgerData.bills.length} 笔账单。`;
      renderApp();
      ledgerRoot.dispatchEvent(new CustomEvent("travel-ledger:changed", {
        bubbles: true,
        detail: { tripId: ledgerTripId, reason: "backup-imported", data: deepClone(ledgerData) }
      }));
    } catch (error) {
      console.error("TravelLedger could not import backup", error);
      setNotice("导入失败，当前账本未被替换。");
    }
  }
      ...
      <dialog class="ledger-dialog ledger-settings-dialog" data-ledger-dialog="settings" aria-labelledby="ledger-settings-dialog-title">
        ...
        <div class="ledger-dialog-body">
          <section class="ledger-setting-group">
            ...
            ${baseLocked ? `<p class="ledger-setting-note">原币金额分别保存，最终统一以人民币结算。</p>` : ""}
          </section>
          <section class="ledger-setting-group">
            <div class="ledger-setting-heading">
          ...
          <section class="ledger-setting-group ledger-backup-group">
            <div class="ledger-setting-heading">
              <div><h3>账本备份</h3><p>换网址或换设备时，用 JSON 文件迁移全部成员、账单和汇率设置</p></div>
            </div>
            <div class="ledger-backup-actions">
              <button class="ledger-secondary-button" type="button" data-ledger-action="export-backup">导出账本</button>
              <button class="ledger-secondary-button" type="button" data-ledger-action="import-backup">导入账本</button>
              <input class="ledger-visually-hidden" type="file" accept="application/json,.json" data-ledger-backup-input>
            </div>
            <p class="ledger-setting-note ledger-backup-note">首次从旧网址迁移：旧网址导出一次 → 新网址导入一次。导入会覆盖当前浏览器里的账本。</p>
          </section>
        </div>
      </dialog>`;
  }
  ...
  function syncSplitSummary() {
    ...
    const amountField = form.elements.originalAmount;
    ...
    summary.textContent = `已选 ${participants.length} 人 · 每人约 ${formatMoney(averageCents, currency)}`;
  }
  ...
  async function submitBill(form) {
    ...
    const baseAmountCents = isForeign ? null : originalAmountCents;
  ...
  function handleAction(button) {
    ...
    if (!action) return;
    if (action === "refresh-exchange") {
      void refreshExchange();
    } else if (action === "toggle-manual-rate") {
      const code = String(button.dataset.ledgerCode || "").toUpperCase();
      manualRateEditorCurrency = manualRateEditorCurrency === code ? "" : code;
      renderApp();
      if (manualRateEditorCurrency) requestAnimationFrame(() => ledgerRoot.querySelector('[data-ledger-form="manual-rate"] input[name="rate"]')?.focus({ preventScroll: true }));
    } else if (action === "clear-manual-rate") {
      void clearManualRate(button.dataset.ledgerCode || "");
  ...
  async function handleRootChange(event) {
    if (event.target.matches("[data-ledger-backup-input]")) {
      await importLedgerBackup(event.target);
      return;
    }
  ...
  async function handleRootSubmit(event) {
    ...
    if (form.dataset.ledgerForm === "manual-rate") {
      await submitManualRate(form);
      return;
    }
  ...
  function setActiveTab(tab, options = {}) {
    ...
    activeTab = nextTab;
    if (changed && nextTab === "stats") void refreshExchange();
  ...
  async function init(options = {}) {
    ...
    if (activeTab === "stats") void refreshExchange();
